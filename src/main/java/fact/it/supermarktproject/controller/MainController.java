...
public class MainController {
    ...
    public String welkom(Model model, HttpServletRequest request){
        ...
        Klant k = new Klant(naam, familienaam);
        k.setGeboortejaar(geboortejaar);
        model.addAttribute("klant", k);
        return "2_welkom";
    }

    @RequestMapping("/nieuwpersoneelslid")
    public String nieuwpersoneelslid(){
        return "3_nieuwpersoneelslid";
    }

    @RequestMapping("/welkompersoneelslid")
    public String welkompersoneelslid(Model model, HttpServletRequest request){
        String naam = request.getParameter("naam");
        String familienaam = request.getParameter("familienaam");
        String dienstjaarStr = request.getParameter("dienst");

        DateTimeFormatter format = DateTimeFormatter.ofPattern("dd/MM/yyyy");
        LocalDate dienstjaar = LocalDate.parse(dienstjaarStr, format);

        Personeelslid p = new Personeelslid(naam, familienaam);
        p.setInDienstSinds(dienstjaar);
